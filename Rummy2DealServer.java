...
public class Rummy2DealServer extends CasinoGame {
	static Logger _cat = Logger.getLogger(Rummy2DealServer.class.getName());
	String _name;
	double _minBet;
	double _maxBet;
	...
	public Rummy2DealServer(String name, double minB, double maxB,
	...
	public StringBuffer gameDetail() {
		...
		sb = new StringBuffer("Rummy2DealServer=Rummy2DealServer").append(",min-bet=")
	...
	class RummyDealTable implements Runnable {
		...
		public void handleGameJoinReq(int obspos, String name) {
			...
			if (startNewRound && !rejoinReq){
				_cat.debug("game ongoing. new player can't join.");
				return;
			}
			...
			if (!rejoinReq) {
				if (!rejoinReq) {
					...
					if (countPlayersInit >= 2 || fixingDealerOngoing
						...
						_cat.debug("game join calling resettable with false");
			...
			addPresence(p);
			System.out
					.println("Rummy2DealServer game - sit in req buf -- on table : "
							+ _tid);
			...
			if (!_gameOngoing) {
				...
				if (getCntStatusActivePlayers() >= 2)
					fixDealerFirst();
				else
					broadcastMessage(-1);
			}
...
//			_cat.debug("getcntstatusactpl : ");
		...
		public void saveWinners(int index) {
			...
			try {
				...
				for (int i = 0; i < listWinners.size(); i++) {
					...
					_cat.debug("pr : " + pr.name + ", amt : " + pr.getRUMMYWinAmt());
					...
					grs.setAmount(pr.getRUMMYWinAmt());
		...
		public void declareGameOver(){
			...
			_cat.debug("calling resettable with false");
			resetTable(false);
				...
				if (!_gameOngoing
					...
					if (getCountGamesPerRound() < (MAX_GAMES_PER_ROUND) && (System.currentTimeMillis() - _nextMoveExpTime) > 10000){
						if (getCountActivePlayers() == 1){
							_cat.debug("run : ending game as only 1 player");
							declareGameOver();
						}
						else {
						...
						_cat.debug("run : time to wrap up the game. all deals are done. send game result now.");
						declareGameOver();
					}
				}

				if (!_gameOngoing && _nextMoveExpTime != -1
						&& System.currentTimeMillis() - _nextMoveExpTime > 10000 &&
						!startNewRound) {
					...
					if (getCountTotalPlayers() >= 2 && counterGameNotStarted < 5) {
						if (countPlayers + getCountNextGameJoin() + getCountSitInReqPlayers() >= 2){
							...
							_cat.debug("run : calling resettable with false");
							resetTable(false);
							if (countPlayers == 1)
								broadcastMessage(-1);
						}
...
//				_nextMoveExpTime = System.currentTimeMillis();
		...
		public void fixDealerFirst() {
			...
			_cat.debug("fixdealer : calling resettable with true");
			...
			if (active <= 1){
				...
				if (breakoutMatch > 0 || (getCountGamesPerRound() > 0 && getCountGamesPerRound() <= MAX_GAMES_PER_ROUND)
						|| startNewRound){
					_cat.debug("fixdealer : ending game...");
		...
		public void startGameIfPossible() {
			// game can begin ---
			// now clear out the all in players list
			_cat.debug("startgame : calling resettable with true");
			resetTable(true);
...
//	            String str = new String(encrypted, "ISO-8859-1");
		...
		public void broadcastMessage(int winnerPos) {
			...
			temp.append("Rummy2DealServer=Rummy2DealServer");
			...
			if (!startNewRound){
				double to = 10000 - (System.currentTimeMillis() - _nextMoveExpTime);
				if (to < 0)
					to = 0;
...
//	}
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAX_TABLES || tid < 0 || !_tables[tid].validTable) {
			buf.append("Rummy2DealServer=Rummy2DealServer,grid=")
					.append(-1)
		...
		buf.append("Rummy2DealServer=Rummy2DealServer,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("Rummy2DealServer game - sit in req --" + movedet);
...
//				}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("Rummy2DealServer game - move req --" + movedet);

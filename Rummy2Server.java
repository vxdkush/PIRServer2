...
public class Rummy2Server extends CasinoGame {
	static Logger _cat = Logger.getLogger(Rummy2Server.class.getName());
	String _name;
	double _minBet;
	double _maxBet;
	...
	public Rummy2Server(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		_minBet = minB;
...
//				_tables[i].POINTSPERRUPEE = 10;
	...
	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("Rummy2Server=").append(_name).append(",min-bet=")
		...
		public void resetTable() {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null) {
					...
					//		_cat.debug("status of player : " + _players[m].getRUMMYStatus() + " for pos : " + m);
...
//			_cat.debug("SERVER reset table ACTIVE PLAYERS : " + countActive);
		...
		public void handleGameJoinReq(int obspos, Presence pold) {
			...
			addPresence(p);
			_cat.debug("Rummy2Server game - sit in req buf -- on table : "
							+ _tid);
...
//	            String str = new String(encrypted, "ISO-8859-1");
		...
		public void broadcastMessage(int winnerPos) {
			...
			temp.append("Rummy2Server=Rummy2Server");
...
//				_cat.debug(decrypt(strcrypt));
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAX_TABLES || tid < 0 || !_tables[tid].validTable) {
			buf.append("Rummy2Server=Rummy2Server,grid=")
					.append(-1)
		...
		buf.append("Rummy2Server=Rummy2Server,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("Rummy2Server game - sit in req --" + movedet);
		...
		if (tid >= MAX_OPEN_TABLES){
			...
			if (moveType == 0){
				...
				for (int i = 0; i < toks.length; i++){
					...
					buf2.append("Rummy2Server=Rummy2Server,grid=")
					.append(-1)
					.append(",TID=").append(tid)
					.append(",URInvited=1")
...
//				}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("Rummy2Server game - move req --" + movedet);

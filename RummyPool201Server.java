...
public class RummyPool201Server extends CasinoGame {
	static Logger _cat = Logger.getLogger(RummyPool201Server.class.getName());
	String _name;
	double minBet;
	double maxBet;
	...
	public RummyPool201Server(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		minBet = minB;
		...
		for (int i = 0; i < MAXTABLES; i++) {
			...
			_cat.debug("starting pool 201 thread : " + i);
		}
	}

	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("RummyPool201Server=RummyPool201Server").append(",min-bet=")
				...
				for (int m = 0; m < players.length; m++){
					if (players[m] != null && !isLost(players[m]) && !isLeft(players[m]) && !isRemoved(players[m])
						...
						if (players[m].totalScore > 174){
							_cat.debug("highest score of some players is 175 or more. can't join now");
...
//			p.setPresence(pold);
			...
			addPresence(p);
			System.out
					.println("RummyPool201Server game - sit in req buf -- on table : "
							+ tid);
...
//	            String str = new String(encrypted, "ISO-8859-1");
			...
			public void broadcastMessage(int notUsedPos) {
				...
				temp.append("RummyPool201Server=RummyPool201Server");
...
//			 }
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAXTABLES || tid < 0 || !tables[tid].validTable) {
			buf.append("RummyPool201Server=RummyPool201Server,grid=")
					.append(-1)
		...
		buf.append("RummyPool201Server=RummyPool201Server,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("RummyPool201Server game - sit in req --" + movedet);
...
//			}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("RummyPool201Server game - move req --" + movedet);

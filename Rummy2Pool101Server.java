...
public class Rummy2Pool101Server extends CasinoGame {
	static Logger _cat = Logger.getLogger(Rummy2Pool101Server.class.getName());
	String _name;
	double minBet;
	double maxBet;
	...
	public Rummy2Pool101Server(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		minBet = minB;
		...
		for (int i = 0; i < MAXTABLES; i++) {
			...
			_cat.debug("starting 2 pool 101 thread : " + i);
		}
	}

	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("Rummy2Pool101Server=Rummy2Pool101Server").append(",min-bet=")
...
//			p.setPresence(pold);
			...
			addPresence(p);
			System.out
					.println("Rummy2Pool101Server game - sit in req buf -- on table : "
							+ tid);
...
//	            String str = new String(encrypted, "ISO-8859-1");
			...
			public void broadcastMessage(int notUsedPos) {
				...
				temp.append("Rummy2Pool101Server=Rummy2Pool101Server");
...
//			 }
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAXTABLES || tid < 0 || !tables[tid].validTable) {
			buf.append("Rummy2Pool101Server=Rummy2Pool101Server,grid=")
					.append(-1)
		...
		buf.append("Rummy2Pool101Server=Rummy2Pool101Server,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("Rummy2Pool101Server game - sit in req --" + movedet);
...
//			}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("Rummy2Pool101Server game - move req --" + movedet);

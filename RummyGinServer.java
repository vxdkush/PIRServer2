...
public class RummyGinServer extends CasinoGame {
	static Logger _cat = Logger.getLogger(RummyGinServer.class.getName());
	String _name;
	double _minBet;
	double _maxBet;
	...
	public static final int status_KNOCKED = 4;
	...
	public RummyGinServer(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		_minBet = minB;
	...
	//	_cat.debug(this);
		...
		_tables = new RummyGinTable[MAX_TABLES];
		for (int i = 0; i < MAX_TABLES; i++) {
			_tables[i] = new RummyGinTable();
			_tables[i].validTable = true;
			_tables[i].POINTSPERRUPEE = 1.0;
			Thread t = new Thread(_tables[i]);
			t.setName("RummyGin-Table-" + i);
	...
	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("RummyGinServer=RummyGinServer")
				.append(",min-bet=").append(_minBet).append(",max-bet=")
				.append(_maxBet).append(",RummyGinTables=");
	...
	class RummyGinTable implements Runnable {
...
//		boolean fixingDealerNextHand = false;
		...
		// round winners
		String[] roundWinners = new String[2];
		
		int[] roundWonCount = new int[2];

		int[] totalPtsWon = new int[2];
		...
		public void resetTable() {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null && !isRemoved(_players[m]) && !isLeft(_players[m])) {
					_players[m].getPresence().player().setActiveGame(2);
		...
		public void handleGameJoinReq(int obspos, Presence pold) {
			...
			if (!rejoinReq) {
				...
				if (_countPlayersInit >= 2 || fixingDealerOngoing
					...
					_players[pos].getPresence().player().setActiveGame(2);
			...
			addPresence(p);
			System.out
					.println("RummyGinServer game - sit in req buf -- on table : "
							+ _tid);
...
//			fixingDealerNextHand = true;
		...
		/*	if (!checkGameOver()) {
				// game has to be over because there are only 2 players on
				// table, if it comes here something went wrong
				_gameOngoing = false;
				removePlayersDisconnected();
				_nextMovePlayer = -1;
			}*/
		...
		public String declareRoundOver(int posWinner) {
			...
			if (posWinner == -1) {
				...
				_nextMovePlayer = 222; // special number to denote an attempt at
										// drawing the round
				...
				if (countPlayerResponseDeclareLoser == 0) {
					...
					String str = (roundNo + 1) + "'" + highIndex + "'" + pts[highIndex] + "'" + valEarned
					...
					String str2 = (roundNo + 1) + "'" + otherIndex + "'" + pts[otherIndex]+"'0'0'"
				...
				if (idPlayerKnocking != -1) {
					...
					if (_players[idPlayerKnockedUpon].getRUMMYPoints() <= _players[idPlayerKnocking]
							.getRUMMYPoints()) {
						// deadwood of knocked upon player is greater than
						// deadwood of knocking player - yay!!!!
						valEarned = _players[idPlayerKnocking].getRUMMYPoints()
								- _players[idPlayerKnockedUpon]
										.getRUMMYPoints();
						str = (roundNo + 1)
								+ "'" + idPlayerKnockedUpon + "'" + _players[idPlayerKnockedUpon].getRUMMYPoints() + "'" + valEarned
								+ "'25'"
								+ (totalPtsWon[idPlayerKnockedUpon] + 25 + valEarned)
								+ "'2";
						
						if (!roundWinners[idPlayerKnockedUpon].isEmpty())
							roundWinners[idPlayerKnockedUpon] += "|" + str;
						else
							roundWinners[idPlayerKnockedUpon] = str;

						str2 = (roundNo + 1)
								+ "'" + idPlayerKnocking+ "'" + _players[idPlayerKnocking].getRUMMYPoints() + "'0'0'"
								+ totalPtsWon[idPlayerKnocking] + "'2";

						if (!roundWinners[idPlayerKnocking].isEmpty())
							roundWinners[idPlayerKnocking] += "|" + str2;
						else
							roundWinners[idPlayerKnocking] = str2;

						idPlayerValidDeclared = idPlayerKnockedUpon;

						totalPtsWon[idPlayerValidDeclared] += (valEarned + 25);
						roundWonCount[idPlayerValidDeclared]++;
						
						if (idPlayerKnockedUpon == 0)
							_currRoundResStr = str + "|" + str2;
						else
							_currRoundResStr = str2 + "|" + str;
					} else {
						valEarned = _players[idPlayerKnockedUpon]
								.getRUMMYPoints()
								- _players[idPlayerKnocking].getRUMMYPoints();
						str = (roundNo + 1)
								+ "'" + idPlayerKnocking+ "'" + _players[idPlayerKnocking].getRUMMYPoints() + "'" + valEarned
								+ "'0'" + (totalPtsWon[idPlayerKnocking]+valEarned) + "'1";
						
						if (!roundWinners[idPlayerKnocking].isEmpty())
							roundWinners[idPlayerKnocking] += "|" + str;
						else
							roundWinners[idPlayerKnocking] = str;

						str2 = (roundNo + 1) + "'" + idPlayerKnockedUpon + "'" + _players[idPlayerKnockedUpon].getRUMMYPoints()
								+ "'0'0'" + totalPtsWon[idPlayerKnockedUpon]
								+ "'1";
						
						if (!roundWinners[idPlayerKnockedUpon].isEmpty())
							roundWinners[idPlayerKnockedUpon] += "|" + str2;
						else
							roundWinners[idPlayerKnockedUpon] = str2;
					...
					int otherIndex = idPlayerValidDeclared == 0 ? 1 : 0;
					if (idPlayerValidDeclared == 0)
						resStr = roundWinners[idPlayerValidDeclared] + "`" + roundWinners[otherIndex];
					else
						resStr = roundWinners[otherIndex] + "`" + roundWinners[idPlayerValidDeclared];
				} else {
					// case where player declaring gin wins
					int highIndex = idPlayerValidDeclared;
					_roundWinnerName = _players[idPlayerValidDeclared].name;
					
					int otherIndex = (highIndex == 0) ? 1 : 0;
					int valEarned = _players[otherIndex].getRUMMYPoints();
					totalPtsWon[highIndex] += valEarned + 25;
					
					_cat.debug("total pts : " + totalPtsWon[highIndex] + " , valEarned : " + valEarned + " with bonus");

					String str = (roundNo + 1) + "'"
							+ highIndex + "'" + _players[highIndex].getRUMMYPoints() + "'" + valEarned + "'25'"
					...
					String str2 = (roundNo + 1)
							+ "'" + otherIndex + "'" + _players[otherIndex].getRUMMYPoints() + "'0'0'"
							+ totalPtsWon[otherIndex] + "'0";
					...
					_cat.debug("string for loser : " + roundWinners[otherIndex]);
			...
			if (totalPtsWon[0] >= 100 || totalPtsWon[1] >= 100) {
				// game over, roll the ball for end of game
				int winnerIndex = totalPtsWon[0] >= 100 ? 0 : 1;
				int otherIndex = winnerIndex == 0 ? 1 : 0;

				int ptsEarned = totalPtsWon[winnerIndex]
						- totalPtsWon[otherIndex];

				String strBig = "^" + winnerIndex + "'" + ptsEarned + "'100'";// 100
																				// is
																				// the
																				// game
																				// win
				...
				int linebonus = 0, linebonus2 = 0;
				// now determine the line bonus, 25 for every round win
				linebonus = roundWonCount[winnerIndex] * 25;
		...
		public void declareGameOver(int posWinner, int totalWinPts,
				String winnerString) {
			_nextMoveExpTime = System.currentTimeMillis();
			_gameOngoing = false;
			...
			int otherIndex = _winnerPos == 0 ? 1 : 0;
			
			_players[otherIndex].getPresence().currentRoundBet(winamt + _rake);
			_players[otherIndex].setRUMMYMovesStr("&Penalty^-" + totalWinPts);
			
			_amtWonString += otherIndex + "'" + ((winamt + _rake) * -1) + "|";
			
			//now for both, add winner info
			_players[_winnerPos].setRUMMYMovesStr("&Winner^" + _players[_winnerPos].getName());
			_players[otherIndex].setRUMMYMovesStr("&Winner^-" + _players[_winnerPos].getName());
			...
			new Thread(){
				public void run(){
					...
					GameRunSession grs = new GameRunSession(_gid, rummygrid,GameType.RummyGin);//32768 for gin rummy
//							_type.intVal());
			...
			if (cntActPlyrs < 2) {
				...
				if (cntActPlyrs >= 1) {
					...
					if (pos != -1 && _gameOngoing) {
						...
						int valEarned = computePoints2(_players[otherIndex]._allotedCards);
						totalPtsWon[winnerIndex] += valEarned;
						
						//winning player won't pay for his deadwood _players[winnerIndex].getRUMMYPoints()
						String str = (roundNo + 1)
						...
						_players[otherIndex].setRUMMYPoints(valEarned);

						String str2 = (roundNo + 1) + "'" + otherIndex + "'" + _players[otherIndex].getRUMMYPoints()
						...
						int ptsEarned = totalPtsWon[winnerIndex] - totalPtsWon[otherIndex];

						String strBig = "^" + winnerIndex + "'" + ptsEarned
								+ "'100'";// 100 is the game win bonus
						...
						String resStr = "";
						if (otherIndex == 1) {
							resStr = roundWinners[winnerIndex] + "`" + roundWinners[otherIndex];
							_currRoundResStr = str + "|" + str2;
						}
						else {
							resStr = roundWinners[otherIndex] + "`" + roundWinners[winnerIndex];
							_currRoundResStr = str2 + "|" + str;
						}
		...
		void removePlayersDisconnected() {
			for (int i = 0; i < MAX_PLAYERS; i++) {
				if (_players[i] != null) {
					...
					// else remove him
					if (isRemoved(_players[i]) || isLeft(_players[i])) {
		...
		public void run() {
			while (_keepServicing) {
				...
				if (_gameOngoing
						...
						&& (System.currentTimeMillis() - _nextMoveExpTime) > 25000) { 
					noMovesToBeAccepted = true;
					errorCode = -1;
					if (idPlayerKnocking != -1) {
						idPlayerKnockedUpon = idPlayerKnocking == 0 ? 1 : 0;
					}
					...
					//set the penalty of the other player who has timed out
					int valEarned = computePoints2(_players[_lastMovePos]._allotedCards);
					_players[_lastMovePos].setRUMMYPoints(valEarned);
...
//				}
				...
				// start new round after 8 seconds of result display
				if (_gameOngoing
						&& _nextMovePlayer == -1
...
//						_cat.debug("from run method, calling fixdealerfirst, 8 secs elapsed........");
						if (countPlayers >= 2)
							fixDealerFirst();
						else {
...
//				
//				chatOn = 0;
//				chatMessage = prp.name + " at pos : " + prp.pos() + " asked for draw.";
//
...
//			}
			...
			if (moveId == move_RUMMY_ADJUST_LOSER) {
				//bug - gin player sends this message and creates havoc
				if (pos == idPlayerValidDeclared || pos == idPlayerKnocking){
					_cat.debug("how are you sending this message? " + pos);
					return;
				}
			...
			if (moveId == move_RUMMY_KNOCK) {
				...
				if (flagGameOver) {
					...
					_nextMovePlayer = getNextActivePos(pos);
					broadcastMessage(-1);
			...
			if (moveId == move_RUMMY_DECLAREGIN) {
				...
				if (flagGameOver) {
					...
					// serious mistake on client side, can't come here
					chatOn = 0;
					chatMessage = "Pos " + (pos + 1) + " made invalid gin call.";
					prp.setRUMMYMovesStr("^Invalid~" + result);
					sendErrorMessage(pos, 4);
					_nextMovePlayer = getNextActivePos(_nextMovePlayer);
					_nextMovesAllowed = move_RUMMY_NEWCARD;
...
//				_nextMoveExpTime = System.currentTimeMillis();
				...
				prp._allotedCards = new Card[11];
				for (int i = 0; i < 10; i++) {
					prp._allotedCards[i] = clonedCards[i];
				}
				prp._allotedCards[10] = cr;
			...
			if (moveId == move_RUMMY_PASS) {
				...
				if (typeO == 0) {
					...
					prp._allotedCards[10] = _firstCard;
					_firstCard = null;
			...
			if (_countPlayersInit >= 2) {
				...
				for (int m = 0; m < MAX_PLAYERS; m++) {
					...
					// && _players[m].getPresence() != null
						...
						_players[m]._firstMoveDone = false;
						_players[m]._allotedCards = new Card[10];
		...
		public void startNewRound() {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null && _players[m].getPresence() != null) {
					...
					_players[m].cardsStrFromClient = "";
					_players[m]._allotedCards = new Card[10];// always clear
																// the hand
																// here
...
//	            String str = new String(encrypted, "ISO-8859-1");
		...
		public void broadcastMessage(int noUsePos) {
			...
			temp.append("RummyGinServer=RummyGinServer");
...
//			_cat.debug("temp haha : " + temp);

			if (idPlayerKnockedUpon != -1) {
				temp.append(",KnockedOnPlayer=").append(idPlayerKnockedUpon);
			}
...
//			_cat.debug("temp hoohoo : " + temp + " , winnerpos : " + _winnerPos + ", fixingin dealer : " + fixingDealerOngoing);
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAX_TABLES || tid < 0 || !_tables[tid].validTable) {
			buf.append("RummyGinServer=RummyGinServer,grid=")
					.append(-1)
		...
		buf.append("RummyGinServer=RummyGinServer,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("RummyGinServer game - sit in req --" + movedet);
...
//				}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("RummyGinServer game - move req --" + movedet);

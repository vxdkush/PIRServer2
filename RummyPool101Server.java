...
public class RummyPool101Server extends CasinoGame {
	static Logger _cat = Logger.getLogger(RummyPool101Server.class.getName());
	String _name;
	double minBet;
	double maxBet;
	...
	public RummyPool101Server(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		minBet = minB;
		...
		tables = new RummyPoolOneTable[MAXTABLES];
		for (int i = 0; i < MAXTABLES; i++) {
			tables[i] = new RummyPoolOneTable();
			tables[i].validTable = true;
			tables[i].POINTSPERRUPEE = (i + 1) * 1;
			

			Thread t = new Thread(tables[i]);
			t.setName("RummyPool1-Table-" + i);
			t.setPriority(Thread.NORM_PRIORITY);
			tables[i].setIndex(i);
			t.start();

			_cat.debug("starting pool 101 thread : " + i);
		}
	}

	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("RummyPool101Server=RummyPool101Server").append(",min-bet=")
	...
	class RummyPoolOneTable implements Runnable {
...
//						}
						...
						if (gameOngoing && players[m].totalScore >= 101){
...
//			}
		...
		public synchronized void handleGameJoinReq(int obspos, String name) {
			...
			if (!rejoinReq) {
				...
				pos = p.pos;
				//bug : if rejoin comes again, do not send that one player has more than 80
				...
				//TBD - check condition of points being less than 79 here.....
				for (int m = 0; m < players.length; m++){
					if (players[m] != null && !isLost(players[m]) && !isLeft(players[m]) && !isRemoved(players[m])
						...
						if (players[m].totalScore > 79){
							_cat.debug("highest score of some players is 80 or more. can't join now");
...
//			p.setPresence(pold);
			...
			addPresence(p);
			System.out
					.println("RummyPool101Server game - sit in req buf -- on table : "
							+ tid);
...
//			else if (countPlayers == 2 && !gameOngoing) {
//				winnerPos = -1;
		...
		public int getHighestScore(){
			...
			for (int k = 0; k < players.length; k++){
				if (players[k] != null && !isToJoinNextGame(players[k]) && !isLost(players[k])
						&& !isRejoined(players[k]) && players[k].totalScore < 101
						){
		...
		private void handleMoveNoAction(int pos) {
			...
			if (nextMovePlayer != 111) {
				...
				if (players[pos].firstMoveDone)
					players[pos].setRUMMYPoints(40);
				else {
					players[pos].setRUMMYPoints(20);
...
//					removePlayersDisconnected();
		...
		private void handleTimedOut() {
			if (players[nextMovePlayer].firstMoveDone)
				players[nextMovePlayer].setRUMMYPoints(40);
			else {
				players[nextMovePlayer].setRUMMYPoints(20);
...
//						&& !isRejoined(players[m])
		...
		private int determineGameOver(){
			...
			if (countActive >= 2){
				//for auto split
				//check if both players have more than >= 80
			...
			if (countActive == 3){
				...
				if (countPotContenders > 3 || (countPotContenders == 3 && numRejoins == 1)){
					...
					//so a player with 80 will have 1 drop count for adding 20 will take him to 100 and still safe
					...
					for (int i = 0; i < players.length; i++){
						if (players[i] != null && isActive(players[i]) && !isToJoinNextGame(players[i]) && !isLost(players[i])){
							int dropcount = 0;
							while ((players[i].totalScore + ((dropcount+1) * 20)) < 101)
								dropcount++;
		...
		//					type.intVal());
			...
			if (gameOverFlag){
				...
				for (int m = 0; m < MAXPLAYERS; m++){
					if (players[m] != null && !isRemoved(players[m]) && 
							!isLeft(players[m]) && !isSittingOut(players[m]) &&
							!isLost(players[m]))
						players[m].setRUMMYStatus(statusACTIVE);
				}
...
//				}
				...
				if (!gameOngoing && nextMoveExpTime != -1
						&& System.currentTimeMillis() - nextMoveExpTime > 60000) {
					...
					if (getCountTotalPlayers() >= 2 && counterGameNotStarted < 1) {
						...
						if (System.currentTimeMillis() - nextMoveExpTime > 75000) {
							counterGameNotStarted = 0;
						}
						
						// remove the removed players now
						if (System.currentTimeMillis() - nextMoveExpTime > 77000) {
							for (int m = 0; m < MAXPLAYERS; m++) {
								if (players[m] != null) {
									sendMessageKickedOut(m, 2);
									players[m] = null;
		...
		private void handleManualSplit(){
			...
			for (int i = 0; i < players.length; i++){
				if (players[i] != null && isActive(players[i]) && !isToJoinNextGame(players[i])
					...
					int dropcount = 1;
					while ((players[i].totalScore + ((dropcount) * 20)) < 101)
						dropcount++;
...
//			rummygrid = setNextGameRunId();
			...
			cardDealingOrder = "";
			for (int m = 0; m < MAXPLAYERS; m++) {
				int index = dealer - (m + 1);
				if (index < 0)
					index = 5 + index + 1;
				if (players[index] != null
						&& players[index].getPresence() != null
						&& isActive(players[index]))
					cardDealingOrder += index + "'";
			}
			cardDealingOrder = cardDealingOrder.substring(0,
					cardDealingOrder.length() - 1);
...
//	            String str = new String(encrypted, "ISO-8859-1");
			...
			public void broadcastMessage(int notUsedPos) {
				...
				temp.append("RummyPool101Server=RummyPool101Server");
...
//					_cat.debug(decrypt(strcrypt));
				...
				String lostStr = "";
				if (!winnerString.isEmpty() && amtWonString.isEmpty()) {
...
//			 }
		...
		public void sendErrorMessage(int prpos, int resCode) {
			...
			if (resCode == 0) {
				...
				temp.append(",MsgDropped=").append("Score-80");
			} else if (resCode == 6) {
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAXTABLES || tid < 0 || !tables[tid].validTable) {
			buf.append("RummyPool101Server=RummyPool101Server,grid=")
					.append(-1)
		...
		buf.append("RummyPool101Server=RummyPool101Server,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("RummyPool101Server game - sit in req --" + movedet);
...
//			}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("RummyPool101Server game - move req --" + movedet);
	...
	public void leave(Presence p) {
		...
		if ((kp.getRUMMYStatus() == statusACTIVE) || (kp.getRUMMYStatus() == statusFOLDED) || (kp.getRUMMYStatus() == statusREJOINED)
				|| (kp.getRUMMYStatus() == statusLEFT) || kp.getTotalScore() > 0){
			...
			tables[tid].players[kp.pos()] = null;
		}
	}
}

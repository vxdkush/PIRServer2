...
public class Rummy221Server extends CasinoGame {
	static Logger _cat = Logger.getLogger(Rummy221Server.class.getName());
	String _name;
	double _minBet;
	double _maxBet;
	...
	public Rummy221Server(String name, double minB, double maxB, GameType type,
			int gid) {
		_gid = gid;
		_name = name;
		_minBet = minB;
	...
	public StringBuffer gameDetail() {
		StringBuffer sb;
		sb = new StringBuffer("Rummy221Server=").append(_name)
...
//		int countHandsAfterDealerFix = 0;
		...
		public void handleGameJoinReq(int obspos, Presence pold) {
			...
			addPresence(p);
			System.out
					.println("Rummy221Server game - sit in req buf -- on table : "
							+ _tid);
...
//			fixingDealerNextHand = true;
		...
		public void declareGameOver(int posWinner) {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null
						&& (isActive(_players[m]) || isLeft(_players[m]) || isFolded(_players[m]))) {
					if (m != _winnerPos) {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null
						&& ((isActive(_players[m]) && !isFolded(_players[m])))) {
			...
			for (int m = 0; m < MAX_PLAYERS; m++) {
				if (_players[m] != null
					...
					for (int k = 0; k < MAX_PLAYERS; k++) {
						if (_players[k] != null
								&& ((isActive(_players[k]) && !isFolded(_players[k])))) {
							if (m != k) {
...
//							_type.intVal());
		...
		int[] checkValidCards2(Card[] cards, int cardsLen, int challenger,
			...
			if (_players[idPlayerChecked].tanalaCount >= 3) {
				System.out
						.println("hurrah ! tanal count to beat all tanala counts : "
			...
			if (_players[idPlayerChecked].tanalaCount == 2
				...
				System.out
						.println("haha : tanala + printed jokers make a great couple");
				resTemp[0] = 0;
				resTemp[1] = 0;
				resTemp[2] = 1;
				return resTemp;
			...
			if (countPureRuns[0] > 0) {
				...
				if (countPureRuns[0] >= 3) {
					...
					// pure runs
					System.out
							.println("we don't have 3 pure runs but we do have : "
									+ countPureRuns);
					Card[] cardsPenal = removeAllCardsOfRun2(cardsStrFrmClnt,
							restrVal);
...
//	            String str = new String(encrypted, "ISO-8859-1");
		...
		public void broadcastMessage(int winnerPos) {
			...
			temp.append("Rummy221Server=Rummy221Server");
...
//			for (int i = 0; i < MAX_PLAYERS; i++) {
	...
	public StringBuffer rummyReload(Player.Presence p, String movedet) {
		...
		if (tid >= MAX_TABLES || tid < 0 || !_tables[tid].validTable) {
			buf.append("Rummy221Server=Rummy221Server,grid=")
					.append(-1)
		...
		buf.append("Rummy221Server=Rummy221Server,grid=").append(p.getGRID())
	...
	public StringBuffer rummySitIn(Player.Presence p, String movedet) {
		...
		_cat.debug("Rummy221Server game - sit in req --" + movedet);
...
//				}
	...
	public StringBuffer rummyMove(Player.Presence p, String movedet) {
		...
		StringBuffer buf = new StringBuffer();// for response back to player
		_cat.debug("Rummy221Server game - move req --" + movedet);
